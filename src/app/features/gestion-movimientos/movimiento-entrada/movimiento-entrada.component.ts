...
export class MovimientoEntradaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (user && user.rol === 'JEFE_ALMACEN' && user.sucursalId) {
      this.entradaForm.patchValue({ sucursalId: user.sucursalId });
      this.entradaForm.get('sucursalId')?.disable();
    }
  }
  ...
  onSubmit() {
    ...
    // Usar getRawValue() para incluir campos deshabilitados (como sucursalId bloqueado para jefe de almacén)
    const formValues = this.entradaForm.getRawValue();
